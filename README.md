...
#### Syntax Highlighting:
Syntax highlighting is where the code within the IDE is changed visually to show the user that there is an issue with the code entered. The IDE would do this visual change according to the category of terms determined by the IDE itself. The IDE does this so that the user can identify a function or an if statement as well as many different coding standards. This can benefit the user as it is will now be easier to find what they may want. The advantages of this is it makes your code easier to understand as people can see the main parts of the program, also as the user you are able to spot problems if they were to occur and something was not to highlight another advantage of syntax highlighting is that is makes it easier to spot errors.
#### Bracket Matching
Bracket matching is available in almost every IDE. The features is there so instead of the user inserting both brackets, the IDE does it for you. The IDE will still allow the code to run if it has not been closed with a bracket so thats why this feature was implimented. This feature is purely to help the user so that the code works, it can also save time for the user if they know how to use it affectivly. Bracket matching is also useful when many nested if statements and program loops are involved. This is because you can easily spot which bracket goes with each one by hovering over it or clicking it. The benefits of Bracket matching are that your code should always have a closed bracket thanks to bracket matching. Also you can see which code is indented to certain parts of the code when using bracket matching. It also makes the code easier to understand when someone else is looking through your code.
#### Step over/into/under
We will start with step into, step into's job is to execute code one statement at a time. This will help the user find errors as they work through their code. Step over is similar to into however step over executes the procedure as a unit and then steps to the next statement in the current procedure. Lastly step out executes the remaining lines of a function in which the current execution point lies. The main problem with step out is that the code has to be in break mode only. All of these features are helpful to do one thing and that is to Debug your code and to make sure that it will run. There are many different ways you can debug your code these three are only some of many but they are probably the most used. The advantages of using Step Over/Into/Out are that you have many different options when it comes to debugging and some ways suit certain parts of code better and with Step Over/Into/Out you have three different options to choose from or even try all three.
#### Auto completion
Lastly we will take a look at auto completion. Auto complete is designed to help save time whilst writting code. If you start to type a function, argument or variable on most IDE's it will come up with a suggestion on how to finish the line. If you are trying to link certain variables, then it will normally give you the options as a drop down. It will do this to see if you want to select it instead of trying out the whole name if it is long. Auto-Completion appears in many program environments, an example implementation being Visual Studio's IntelliSense. The term was originally popularized as picklist. Code-Completion works using an automatically generated in-memory database of classes, variable names, and other constructs that the application being edited defines or references. The advantages of Auto-Completion are, It can save the coder time. It can also simplify linking variables and functions as it will only list ones that are available so if one isn’t then something is wrong.
...
| Step 3a - Execution: | Step 3b - Invoke(Load into memory): |
|If the step above has been completed succesfully then the computer can finally run the program. Each step and instruction has been converted into machine code that the computer can read. Each instruction itself will trigger thousands of actions on the target machine that will all come together to do what the program was originally intended for. The program will be exported as an .exe file or another executable file type.| This process only happens if the previous step (Step 2b) is completed. Once the code has been checked it will then be allocated memory. The operating system will handle this step as it needs to determine how much memory exactly the program needs to run efficently. Once this is done the code will be executed line by line.|
...
**Fitness** - Will the software address and fix the problem, or or is the software being used for an a malicious or unintented purpose?
**Maturity** - Using the first ever version of any piece of software can be very risky. Hardware and software companies are constantly pressured to release or finish a project early before they are completely finished or ready for release. If these companies do rush the development then this can lead to many problems being missed which in turn will mean the released version will have these issues.
**Support** - Companies need to be able to provide support and resolve issues if there products dont meet the intended needs. Some companies will opt for a more open source format in order for users to resolve some issues without direct contact with an advisor from the company.
**License and support costs** - With most mainstream pieces of software there is usually a free or a lower cost version out there to compete. A lot of these companies these days will give users the option to go for a monthly or an annual payment plan. Open source software can be expensive if you are buying directly from the company, especially if the comapny has bad licensing terms.

## User and software requirements:
The software requirements are what are given to you by the client when you have the intial meeting. When you first meet them they will give a presentation or even give you the information on what they would like to see in the product. They usually give you a brief of the project so it will be your job to break it down and create Epics and user stories. You will use these epics and user stories as your mile stones when designing and programming the product. You will also break down User Stories into smaller tasks to follow while you work, this will also help for debugging. There is also the matter of user requirements, these user requirements will be addressed through what the client belives their product will be used for, For instance if the client has asked for a a shopping cart to be added to their site you wouldnt put down that a user requirement would be for the customer to upload photos, the user requirement is what the client wants the user to do with the product, so for our example the user requirement would be to make sure the user can add their desired product to a basket.
